...
func NewScan(urls []string, thread int, output string, proxy string) *FinScan {
	...
	if err != nil {
		color.RGBStyleFromString("238,99,99").Println("[error] finger.json文件读取错误!!!")
		os.Exit(1)
	}
...
func (s *FinScan) StartScan(kwpath string) {
	...
	for _, aas := range s.FocusResult {
		...
		color.RGBStyleFromString("238,99,99").Printf(fmt.Sprintf("%s", aas.Cms))
		fmt.Printf(fmt.Sprintf(" | %s | %d | %d | %s ]\n", aas.Server, aas.Statuscode, aas.Length, aas.Title))
	}
	if s.Output != "" {
		outfile(s.Output, s.AllResult, kwpath)
	}
}
	...
	for s.UrlQueue.Len() != 0 {
		...
		case []string:
			...
			if len(out.Cms) != 0 {
				...
				color.RGBStyleFromString("144,238,144").Println(outstr)
